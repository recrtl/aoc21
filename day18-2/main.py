...
def deepCopy(value):
    if isinstance(value, int):
        ...
        return [deepCopy(value[0]), deepCopy(value[1])]
...
for i in range(0, len(inputs)):
    for j in range(0, len(inputs)):
        if i == j:
            continue
        sum1 = add(deepCopy(inputs[i]), deepCopy(inputs[j].copy()))
        reduce(sum1)
        maxMagnitude = max(maxMagnitude, magnitude(sum1))
        # print(f'{inputs[i]} + {inputs[j]} = {sum1}: {magnitude(sum1)}')

        sum2 = add(deepCopy(inputs[j].copy()), deepCopy(inputs[i]))
        reduce(sum2)
        maxMagnitude = max(maxMagnitude, magnitude(sum2))
        # print(f'{inputs[j]} + {inputs[i]} = {sum2}: {magnitude(sum2)}')
